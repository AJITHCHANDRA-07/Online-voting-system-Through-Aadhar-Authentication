...
def showVotes(root,frame1):
    ...
    bjpLogo = ImageTk.PhotoImage((Image.open("img/bjp.png")).resize((35,35),Image.BILINEAR))
    bjpImg = Label(frame1, image=bjpLogo).grid(row = 2,column = 0)

    congLogo = ImageTk.PhotoImage((Image.open("img/cong.jpg")).resize((25,38),Image.BILINEAR))
    congImg = Label(frame1, image=congLogo).grid(row = 3,column = 0)

    jspLogo = ImageTk.PhotoImage((Image.open("img/jsp.png")).resize((45,30),Image.BILINEAR))
    jspImg = Label(frame1, image=jspLogo).grid(row = 4,column = 0)

    ycpLogo = ImageTk.PhotoImage((Image.open("img/ycp.png")).resize((40,35),Image.BILINEAR))
    ycpImg = Label(frame1, image=ycpLogo).grid(row = 5,column = 0)

    notaLogo = ImageTk.PhotoImage((Image.open("img/nota.jpg")).resize((35,25),Image.BILINEAR))
